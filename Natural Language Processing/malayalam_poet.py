...
corpus = []
corpus = data.split("\n")
[item.encode('utf-8').decode('utf-8') for item in corpus]
print(u"Corpus is \n{corpus}")
...
def create_model(total_words, max_sequence_len):
    ...
    model.add(tf.keras.layers.Dropout(0.1))  # A dropout layer# Another LSTM Layer
    model.add(LSTM(100))
...
for _ in range(next_words):
    ...
    if output_word == prev_word or output_word == last_word:
        probabilities = model.predict(token_list, verbose=0)
        choice = np.random.choice([1, 2, 3])
        predicted = np.argsort(probabilities)[0][-choice]
        output_word = tokenizer.index_word[predicted]
